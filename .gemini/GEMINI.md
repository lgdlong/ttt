# GEMINI Copilot Guidelines for TTT Project
...
## Agent Enforcement for Gemini CLI Tools
- **Khi được giao Task:** Luôn bắt đầu bằng `write_todos` để lập kế hoạch dựa trên `sequential-thinking`.
- **Khi Debug:** Sử dụng `codebase_investigator` để tìm root cause.
- **Khi kết thúc:** BẮT BUỘC chạy `run_shell_command` với các lệnh hoặc `pnpm build` trước khi trả lời "Done".